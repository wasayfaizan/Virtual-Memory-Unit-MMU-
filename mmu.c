...
int bd = open(BackingFileName,O_RDONLY); // file descriptor for BACKING_STORE.bin
 ...
 // Check for Page Fault
    ...
    float pageFaultRate = (float)PageFault * 100 / numaddresses;
    float tlbHitsRate = (float)TBLHits * 100 / numaddresses;

   

    // Append rates to the CSV file
    fprintf(fptr, "Page Faults Rate, %0.2f%%,\n", pageFaultRate);
    fprintf(fptr, "TLB Hits Rate, %0.2f%%,\n", tlbHitsRate);     
